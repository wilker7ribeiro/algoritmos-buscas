...
export class Profundidade {
	...
	constructor(allNodes: Node[], nodeInicial: Node, nodeObjetivo: Node, options?: ProfundidadeOptions) {
		this.nodes = allNodes;
		this.nodeInicial = nodeInicial;
		this.nodeObjetivo = nodeObjetivo;
		this.options = Object.assign(new ProfundidadeOptions(), options);
	}
	...
	expendirNodesVizinhas(itemFronteira: ItemFronteira) {
		itemFronteira.cabeca.vizinhos.forEach(vizinho => {
			...
			if (this.deveIncluirNodeNaFronteira(vizinho.node)) {
				// adiciona o vizinho no início da fronteira com o caminho atualizado	
				let vizinhoCaminho = itemFronteira.nodesCaminho.slice(0);
				vizinhoCaminho.unshift(vizinho.node)
				this.fronteira.unshift(new ItemFronteira(vizinhoCaminho))
			}
		})
	}
	// verifica se deve incluir node vizinha na fronteira
...
export class ProfundidadeOptions {
